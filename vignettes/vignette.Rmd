...
The grouping arguments `sampleLabelsMerged` and `mergeIndex` are optional. If
both are omitted, TSSr assigns each sample to its own analysis group. In that
case no counts are combined and `mergeSamples()` can be skipped. When
biological replicates should be combined, provide both arguments as above.
...
## Combining and splitting raw TSS tables

Raw-count TSS tables from independently processed samples or projects can be
combined directly. Each file may contain one or multiple sample columns. The
same interface can extract selected samples while removing positions that are
zero in every selected column.

```{r TSStable-combine-split}
part1 <- file.path(tempdir(), "TSSr.SL01.SL02.txt")
part2 <- file.path(tempdir(), "TSSr.SL03.SL04.txt")
splitTSSTable(
    exampleInput, c("SL01", "SL02"), part1, overwrite = TRUE
)
splitTSSTable(
    exampleInput, c("SL03", "SL04"), part2, overwrite = TRUE
)
combinedTable <- combineTSSTables(c(part1, part2))
head(combinedTable)
```

The table functions require finite, non-negative integer raw counts. They
reject processed tables and incompatible chromosome or genome metadata rather
than silently combining incomparable values.
...
# peakclu is the published default. peakcluMax is an optional, more
# sensitive greedy candidate method. Use the same clusterThreshold for
# every sample and method included in a comparison.
...
# Export processed TSS values to project1.TSS.processed.txt
exportTSStable(
    myTSSr,
    data = "processed",
    outputPrefix = file.path(tempdir(), "project1")
)
