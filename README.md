...
* Input data files
  ...
  The read alignment files in compressed binary alignment map (BAM) format are required if users intend to call TSSs from raw sequencing data. BAM files can be derived from mapping of either paired-end or single-end TSS sequencing reads.  TSSr also accepts TSS tables generated by TSSr or other bioinformatics tools as input data. 
  A TSS table can be a tab-delimited text file, a BigWig (bw) binary type file, or browser extensible data (bed) type file. A tab-delimited TSS table contains chromosome ID, genomic coordinates, strand information, and raw or normalized read counts of each sample. 
  

* Reference genome

  The reference genome stored as a BSgenome data package must be provided to determine the presence of genome uncoded G at the first site of sequencing read during TSS calling.  For available BSgenome objects, please check (https://bioconductor.org/packages/BSgenome).  In case the desired BSgenome object is unavailable in BSgenome package (not in the list returned by BSgenome::available.genomes() function), a custom genome has to be built and installed before run through this package. (See the vignette “How to forge a BSgenome data package” within the BSgenome package for instructions). Genome annotation file (GTF or GFF file) is required to assign a TSS cluster to a specific gene or genomic feature by using the annotateCluster function. The chromosome names in BSgenome, genome annotation file as well as the reference genome used for mapping of sequencing reads  must be matched.  
...
* To explore the fucntions of TSSr using provided example TSSr object "exampleTSSr". Skip this step if the user plan to creat a new TSSr object using bam or TSS table files 

        myTSSr <- exampleTSSr 
        
* To creat a new TSSr object using the constructer function "new". This function requires user to define the values for "inputFiles", "inputFilesType","genomeName","refSource", "organismName", "sampleLabels", "sampleLabelsMerged", "mergeIndex", reference genome name, input file path, input file type, samples labels, sampleLabelsMerged (if samples are to be merged, e.g., biological replicates), annotation file path (if annotateCluster is to be called), organism name (if annotateCluster is to be called).  The "inputFiles" that contains the path and file names of input files. We provided four BAM files (S01.sorted.bam, S02.sorted.bam, S03.sorted.bam, S04.sorted.bam) obtained from (Lu and Lin 2019) as example data. The CAGE reads were mapped to the reference genome of S. cerevisiae (R64-2-1) using HISAT2 (Kim, Langmead et al. 2015). To reduced file size, each BAM file only includes sequencing reads mapped to two chromosomes (Chr I and Chr II). The example BAM files and TSS table  file can be downloaded from http://www.zlinlab.org/TSSr.html. 

        inputFiles <- c("S01.sorted.bam", "S02.sorted.bam", "S03.sorted.bam", "S04.sorted.bam") # if use TSS table: inputFiles <- "ALL.samples.TSS.raw.txt"
...
* Install required BSgenome object. The example CAGE data was obtained from Saccharomyces cerevisiae. The BSgenome object for S. cerevisiae “BSgenome.Scerevisiae.UCSC.sacCer3” can be obtained from BSgenome Bioconductor package (https://bioconductor.org/packages/BSgenome). 

        if (!requireNamespace("BSgenome.Scerevisiae.UCSC.sacCer3", quietly = TRUE))
        BiocManager::install("BSgenome.Scerevisiae.UCSC.sacCer3")
        library(BSgenome.Scerevisiae.UCSC.sacCer3) 
	
* Define genome annotation gff file. The example genome annotation of S. cerevisiae (R64-2-1) was originally obtained from the Saccharomyces Genome Database: http://zlinlab.org/TSSr/saccharomyces_cerevisiae_R64-2-1.gff 

        gffFile <- "saccharomyces_cerevisiae_R64-2-1.gff" 
	

* Creating a new TSSr object. In this example, we used sample lable "SL01" for "S01.sorted.bam", "SL02" for "S03.sorted.bam", "SL03" for and "SL04" for "S04.sorted.bam". Thus, we define sampleLabels as c("SL01","SL02","SL03","SL04"). If a TSS table is used as input data, the sampleLabels should be the column name of each data. TSSr provide a function to merge TSS signals from biological replicates into a single dataset. SL01 and SL02 are two biological replicates obtained from S. cerevisiae cells grown in rich medium (control), while SL03 and SL04 were obtained by treating S. cerevisiae with α factor (treat). Thus, we set sampleLabelsMerged = c("control","treat") and mergeIndex = c(1,1,2,2), which means SL01 and SL02 are defined as group 1 "control", and SL03 and SL04 are defined as group 2 "treat".
