...
> Note: The variable `localThreshold = 0.02` means that only signals with a minimum TSS intensity greater than 2% of the dominant TSS will be included in the cluster; otherwise, they will be filtered out. Meanwhile, `clusterThreshold = 1` means that a cluster will only be retained if the total TPM (Transcripts Per Million) value exceeds 1. Raising `clusterThreshold` can improve cross-sample support but also removes gene-associated clusters. Full yeast and Arabidopsis benchmarks did not identify a single higher threshold that preserves gene coverage across datasets, so the default remains 1. Use the same explicitly reported value for all samples and methods being compared.

  `peakclu` remains the published default. The optional `peakcluMax` method greedily retains the strongest remaining TSSs using the same open `peakDistance` convention. It generally returns additional weak but multi-TSS candidate clusters and may require more downstream computation; it should not be interpreted as universally more accurate without independent validation.
