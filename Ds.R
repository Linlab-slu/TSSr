...
##.Ds(clustersx.asn,clustersy.asn, librarySizex, librarySizey, useRawCount = TRUE)

############################################################################################################
.Ds <- function(clustersx.asn,clustersy.asn, librarySizex, librarySizey, useRawCount = TRUE){
  setDT(clustersx.asn)
  setDT(clustersy.asn)
  ##clustersx.asn is control
  ##clustersy.asn is test
  clustersx.asn <- clustersx.asn[,c("cluster","strand","dominant_tss","tpm","gene")]
  clustersy.asn <- clustersy.asn[,c("cluster","strand","dominant_tss","tpm","gene")]
  clusters <- merge(clustersx.asn,clustersy.asn, by = c("cluster","strand"), all = T)
