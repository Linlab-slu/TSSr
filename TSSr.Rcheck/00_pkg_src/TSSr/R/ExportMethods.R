...
setMethod("plotCorrelation", signature(object = "TSSr"), function(object, samples) {
    ...
    pdf(
        file = paste("TSS_correlation_plot_of_", paste(samples, collapse = "_"), "_samples.pdf", sep = ""),
...
setMethod("plotInterQuantile", signature(object = "TSSr"), function(object, samples, tagsThreshold) {
    ...
    if (samples == "all") {
        tc <- TCs
        pdf(
            file = paste("Interquantile_plot_of_ALL_samples.pdf", sep = ""),
            width = 8, height = 8, onefile = TRUE, bg = "transparent", family = "Helvetica", fonts = NULL
        )
        for (i in seq_len(length(sampleLabels))) {
            temp <- tc[[sampleLabels[i]]]
            temp <- temp[temp$tags >= tagsThreshold & temp$interquantile_width <= 200, ]
            hist(temp$interquantile_width,
                breaks = 40, col = rainbow(length(sampleLabels))[i],
        ...
        tc <- TCs[[samples]]
        pdf(
            file = paste("Interquantile_plot_of_", paste(samples, collapse = "_"), "_samples.pdf", sep = ""),
            width = 8, height = 8, onefile = TRUE, bg = "transparent", family = "Helvetica", fonts = NULL
        )
        for (i in seq_len(length(samples))) {
            ...
            temp <- temp[temp$tags >= tagsThreshold & temp$interquantile_width <= 200, ]
            hist(temp$interquantile_width,
                breaks = 40, col = rainbow(length(samples))[i],
...
setMethod("plotShape", signature(object = "TSSr"), function(object, samples) {
    ...
    if (samples == "all") {
        tc <- TCs
        pdf(
            file = paste("Shape_plot_of_ALL_samples.pdf", sep = ""),
            width = 8, height = 8, onefile = TRUE, bg = "transparent", family = "Helvetica", fonts = NULL
        )
        for (i in seq_len(length(sampleLabels))) {
            temp <- tc[[sampleLabels[i]]]
            hist(temp$shape.score,
                breaks = 40, col = rainbow(length(sampleLabels))[i],
        ...
        tc <- TCs[[samples]]
        pdf(
            file = paste("Shape_plot_of_", paste(samples, collapse = "_"), "_samples.pdf", sep = ""),
            width = 8, height = 8, onefile = TRUE, bg = "transparent", family = "Helvetica", fonts = NULL
        )
        for (i in seq_len(length(samples))) {
            temp <- tc[[samples[i]]]
...
setMethod("exportClustersToBed", signature(object = "TSSr"), function(object, data, assigned) {
    ...
    for (i in seq_len(length(sampleLabelsMerged))) {
        ...
        writeLines(paste('track name="', sampleLabelsMerged[i],
            ...
            '(TC) q0.1-q0.9)" ',
            'visibility="pack" color=0,255,255',
            sep = ""
        ), df)
