...
.plotTSS <- function(tss, cs, df, samples, Bidirection, up.dis, down.dis, yFixed) {
    ...
    for (my.sample in seq(samples))
    {
        ...
        cs.gr <- makeGRangesFromDataFrame(cs_sub,
        ...
        data_tss_track <- makeGRangesFromDataFrame(temp,
...
.getBed <- function(p) {
    ...
    pbed <- lapply(as.list(seq_len(p[, .N])), function(i) {
        if (p[i, q_0.1] == p[i, q_0.9]) {
            ...
            nrBlocks <- length(unique(c(p[i, start], p[i, end], p[i, q_0.1], p[i, q_0.9]))) - 1
        }
        ...
        if (nrBlocks == 3) {
            ...
            if (p[i, q_0.1] == p[i, start]) {
                ...
                message(paste(utils::capture.output(show(p[i, ])), collapse = "\n"))
            }
        } else {
