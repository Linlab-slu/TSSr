...
.assign2gene <- function(clusters,reference,organim,upstream=1000, upstreamOverlap = 500){
  setDT(clusters)
  clusters[, "subset" := paste0(chr,"_",strand)]
  txdb <- suppressWarnings(makeTxDbFromGFF(reference, organism, format = "auto"))
  ...
  asn <- lapply(as.list(clusters[,unique(subset)]), function(x) {
    ...
    ref_sub <- subset(ref[ref$subset == x,]) ## why ref_sub <- ref[.(x)] not working when x == "chrM_-"?
     if(nrow(ref_sub) == 0){
      ...
      ref_sub$start <- as.numeric(ref_sub$start)
      ...
      if(cs$strand[1] == "+"){
        ...
        ref_sub[,end:= start] ##start - 1 -> start April10
        ref_sub[,start := end - up +1]
      }else{
        ...
        ref_sub[,up:= ifelse(dis > upstream, upstream, 
                             ifelse(dis + width  <= upstreamOverlap, dis + width -1,
                                    ifelse(dis < upstreamOverlap, upstreamOverlap, dis)))]
        ref_sub[,start:= end]##end + 1 -> end April10
        ref_sub[,end:= start + up -1]
      }
